...
상태: **실험 설계 확정 · corpus 확보 전 · 구현/측정 미완료**.

> 같은 이미지 묶음과 1 vCPU·2 GiB 제한에서 두 배포 후보의 처리량, 메모리, 출력 품질과 파일 크기는 어떻게 달라지는가?

이 문서는 구현할 실험의 기술 계약이다. 아래 숫자는 측정할 조건과 호출 수이며 성능 결과가 아니다. 실제 corpus manifest·codec 버전·실행 명령은 준비 단계에서 이 문서와 함께 고정한다. 아직 실행 가능한 E2 runner나 Terraform 환경은 없다.
...
- 두 번째 사진의 정확한 파일·라이선스와 모든 생성 옵션·hash는 corpus 고정 전 확인한다. 추가 사진과 생성 fixture는 아직 확보하지 않았다.

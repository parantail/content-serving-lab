...
첫 비교는 공개 저장소에 넣을 수 있는 원본 한 장과 아래 변환 옵션으로 실행합니다.
...
원본 파일을 정할 때 사용 조건, 파일 크기, 해상도, 포맷과 SHA-256 해시를 함께 기록합니다. 실제 변환기와 인코더 버전도 실행 결과에 남깁니다.

대표 결과를 얻은 뒤에는 큰 JPEG와 알파 채널이 있는 PNG에서도 같은 경향이 나타나는지 확인합니다. 여러 입력을 추가하더라도 첫 결과의 조건을 사후에 바꾸지는 않습니다.

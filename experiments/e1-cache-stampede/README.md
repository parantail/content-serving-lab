...
- **변환 옵션 정규화**: `width=640,height=640,fit=cover,quality=80`과 `quality=80,fit=cover,height=640,width=640`처럼 순서만 다르고 의미가 같은 옵션을 하나의 표현으로 맞추는 일입니다. 정규화된 값이 같으면 같은 파생 이미지 키를 사용합니다. 기술 문서에서는 이를 canonicalization이라고도 합니다.
...
F2의 실행 순서와 결과는 위의 [최초 요청이 취소되면 나머지 요청도 취소되는가?](#f2--최초-요청이-취소되면-나머지-요청도-취소되는가)에서 설명합니다. F5는 실행하지 않았습니다. F3의 질문(서버 변환 제한 시간 초과가 대기 요청에 주는 영향)은 [E3 장애 격리](../e3-failure-isolation/README.md)의 T2 장애로 측정했습니다.

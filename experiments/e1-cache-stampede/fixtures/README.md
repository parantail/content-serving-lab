...
약 16.1 megapixel인 이 원본은 고해상도 JPEG decode와 큰 폭의 축소 작업을 만들면서도 공개 저장소의 첫 fixture로 관리 가능한 크기라서 선택했습니다. 첫 E1 조건은 이 입력을 `640 × 640`, `cover`, WebP quality 80으로 변환합니다. `640 × 640`은 원본 크기가 아니라 card/thumbnail용 파생 이미지 크기입니다. 첫 결과 뒤에는 같은 원본의 `1280 × 1280` 변환을 sensitivity scenario로 실행해 더 큰 파생 이미지에서도 같은 경향이 유지되는지 확인합니다.

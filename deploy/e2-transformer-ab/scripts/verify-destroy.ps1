...
$name="content-serving-e2-$DeploymentId"
$checks=[ordered]@{checked_at=[DateTimeOffset]::UtcNow.ToString('o'); deployment_id=$DeploymentId}
$state=@(Invoke-CheckedCommand terraform @("-chdir=$script:E2Root",'state','list'))
...
$logs=Invoke-AwsJson $Profile $region @('logs','describe-log-groups','--log-group-name-prefix',"/content-serving/e2/$DeploymentId")
...
$configuration=Join-Path $script:E2Root 'local/configuration.json'
...
Save-E2Json (Join-Path $script:E2Root 'local/residual-check.json') $checks
if ($nonzero.Count) { throw "E2 residual resources: $($nonzero.Key -join ', ')" }
Write-Host 'Terraform state and service-specific E2 residual counts are all zero.'

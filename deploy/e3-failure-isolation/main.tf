...
variable "region" {
  ...
  validation {
    ...
    error_message = "E3 runs in Seoul."
  }
}
...
locals {
  name = "content-serving-e3-${var.deployment_id}"
  tags = {
    Project      = "content-serving-lab"
    Experiment   = "e3-failure-isolation"
...
resource "aws_security_group" "runner" {
  name        = local.name
  description = "E3 one-shot runner; no inbound"
  vpc_id      = aws_vpc.experiment.id
}
...
# One private bucket holds originals/, derivatives/ and the uploaded run
# results. The media stores address objects by prefix, so nothing collides.
...
resource "aws_s3_bucket_ownership_controls" "experiment" {
  bucket = aws_s3_bucket.experiment.id
...
# ListBucket lets missing derivative/original keys surface as 404 misses
# instead of 403, matching the E1 store contract.
resource "aws_iam_role_policy" "bucket" {
  name = "originals-derivatives-results"
  role = aws_iam_role.runner.id
  policy = jsonencode({ Version = "2012-10-17", Statement = [
    { Effect = "Allow", Action = ["s3:GetObject", "s3:PutObject"], Resource = "${aws_s3_bucket.experiment.arn}/*" },
    { Effect = "Allow", Action = "s3:ListBucket", Resource = aws_s3_bucket.experiment.arn }
  ] })
}
resource "aws_ecs_task_definition" "runner" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "E3_BUCKET", value = aws_s3_bucket.experiment.id }
    ]
    logConfiguration = {
      logDriver = "awslogs"
      options = {
        ...
        awslogs-stream-prefix = "e3"
      }
    }
  }])
  depends_on = [aws_iam_role_policy.execution, aws_iam_role_policy.bucket]
}
...
output "run_configuration" {
  value = {
    ...
    result_bucket   = aws_s3_bucket.experiment.id

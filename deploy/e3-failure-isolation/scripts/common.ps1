...
$script:E3Root = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path

function Get-E3Runtime {
    Read-RuntimeConfiguration -ModuleRoot $script:E3Root
}
function Get-E3Configuration {
    $raw = Invoke-CheckedCommand terraform @("-chdir=$script:E3Root", 'output', '-json', 'run_configuration')
    ($raw -join "`n") | ConvertFrom-Json
}
function Save-E3Json {
    param([string]$Path, [object]$Value)
    $Value | ConvertTo-Json -Depth 30 | Set-Content -LiteralPath $Path -Encoding utf8NoBOM
}
function Receive-E3Results {
    param([object]$Runtime, [object]$Configuration)
    $recovery = Join-Path $script:E3Root "local/recovered/$($Runtime.deployment_id)"

# AWS E2 선행 실행 — AVIF 품질 설정 오류와 진단 근거

**이 실행은 AVIF 품질 설정 오류로 최종 비교 근거에서 제외했다.** 이후 [수정본 재측정과 최종 선택](../aws-20260910-c2/README.md)을 완료했다. 아래는 결함이 있었던 b1 실행의 원자료와 진단 기록이다.

5회 본 측정과 품질 sweep까지 실행·회수·AWS 제거를 마쳤지만, ImageMagick AVIF의 요청 Q가 encoder에 전달되지 않았다. 같은 실행 이미지의 로컬 진단에서 **요청 Q80 → 실제 libvips Q80 / ImageMagick Q50**을 확인했다. 이 실행으로 AVIF를 포함한 변환기 선택이나 동등 조건 성능을 주장하지 않으며, 수정본 결과와 섞지 않고 원자료를 보존한다.

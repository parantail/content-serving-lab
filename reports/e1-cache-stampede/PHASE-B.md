...
**결정:** 한 프로세스 안에서 같은 이미지 요청을 합치는 현재 방식을 유지합니다. 최초 클라이언트가 요청을 취소해도 서버의 이미지 변환과 다른 클라이언트의 대기는 계속합니다. 후속 [실제 ECS·S3 측정](AWS-S4.md)에서도 Task 간 중복 변환을 확인했습니다. 프로세스 사이의 분산 조정은 이번 E1 범위에서 제외했습니다.

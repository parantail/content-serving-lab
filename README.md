...
| [E3 장애 격리](#e3-장애-격리--변환기와-원본-저장소-장애는-다른-요청까지-영향을-주는가) | 변환기·원본 저장소 장애가 다른 요청까지 영향을 주는가? | 캐시 적중 요청은 오류 0. 다른 이미지의 변환 요청은 17초 대기 → 2초 bounded wait로 2초 안에 503, 복구 지연 11초 → 0초 | bounded wait 채택. kill switch는 영향이 없던 장애에서도 정상 요청의 67%를 거부해 조건부 |

로컬 Docker(1 vCPU·2 GiB)에서 선행 실험과 측정으로 조건을 고정하고 실제 AWS 인프라(ECS Fargate 1 vCPU·2 GiB, S3, ALB, `ap-northeast-2`, 일회성 Terraform)에서 실험을 진행했습니다. AWS 비용은 조회 시점 기준 E1 약 US$0.68, E2 약 US$0.53, E3 약 US$0.09입니다. 부하는 모두 부하 생성기를 통해 만들어진 트래픽입니다.
...
**결정과 한계:** `process-singleflight`를 기본으로 채택했습니다. Task 간 분산 조정(S5)은 이 결과만으로 필요성이 입증되지 않고 잠금 만료·복구 검증이 범위를 크게 넓혀 도입하지 않았습니다. 부하 생성기가 서비스와 CPU를 공유하는 합성 loopback 부하이며, 한 장의 입력과 한 가지 변환만 사용했습니다.
...
**결정과 한계:** libvips를 유지합니다. alpha·JPEG처럼 ImageMagick의 bytes·SSIM 이점이 있는 입력의 비중이 커지면 그 품질점에서 다시 측정합니다. 24입력의 작은 corpus, 특정 codec·preset, 1 vCPU·2 GiB Task 하나의 다섯 반복이며 HTTP 서비스 부하나 tail latency는 측정하지 않았습니다. 첫 AWS 실행은 ImageMagick의 AVIF quality가 실제로 적용되지 않은 것을 quality 조회 gate가 잡아내 폐기하고, 수정·회귀 검증 뒤 다시 실행한 결과입니다.

더 보기: [최종 보고서](reports/e2-transformer-ab/aws-20260910-c2/README.md) · [기술 명세](experiments/e2-transformer-ab/README.md) · [실행 규격](experiments/e2-transformer-ab/RUN.md) · [corpus](experiments/e2-transformer-ab/fixtures/README.md) · [폐기한 첫 실행과 수정 기록](reports/e2-transformer-ab/aws-20260910/README.md) · 코드 [libvips worker](internal/e2vips/transform.go) / [ImageMagick worker](internal/e2magick/transform.go) / [supervisor](internal/e2/) / [분석·차트](experiments/e2-transformer-ab/analyze.py) / [Terraform](deploy/e2-transformer-ab/README.md)
...
- 대응 방식: **M0** baseline(격리 수단 없는 기존 코드) · **M1** bounded wait + shedding(변환 slot 대기에 2초 상한을 두고, 넘긴 요청은 기다리게 하지 않고 즉시 503으로 거절) · **M2** kill switch(운영자가 변환 경로를 끔. miss는 즉시 503, hit은 그대로)
...
- 영향을 받은 것은 다른 이미지의 변환 요청(정상 miss)뿐이었고, 경로는 변환 slot이었습니다. 오염된 요청이 slot 4개를 모두 점유하자 T1/T2에서 baseline은 정상 miss의 93%가 slot을 기다리다 17.3초 만에 응답했고, 대기 요청 25건이 원본 118 MiB를 들고 있어 peak memory가 350 MiB 늘었으며, 장애 종료 뒤에도 11초 동안 영향이 남았습니다.
- M1은 같은 요청을 2초 안에 503(`Retry-After: 1`)으로 끝내고 복구 지연을 0초, memory 증가를 54 MiB로 줄였습니다. 정상 시 비용은 반복 간 변동 안이었습니다.
...
**결정과 한계:** M1을 기본으로 채택하되 원본 읽기는 slot 밖으로 되돌리고 보유 원본 bytes에 별도 상한을 두는 변형을 다음 검토 조건으로 남겼습니다. kill switch는 정상 miss 지표가 실제로 나빠진 것을 확인한 뒤에만 켭니다. 단일 프로세스·단일 Task이고 자동 탐지, 클라이언트 재시도, 여러 Task 사이의 격리는 측정하지 않았습니다.
...
- 서비스, 부하 생성기, 분석기는 모두 이 저장소의 Go 코드이고 실험 image는 base image digest를 고정한 Docker build이며 같은 Dockerfile로 만든 image를 로컬과 AWS에서 실행했습니다.
- 각 보고서는 [같은 순서](docs/experiment-report-format.md)로 씁니다. 질문 → 결과를 보기 전의 예상과 선택 기준 → 환경 → 변경 전 → 변경 → 수치 → 결정 → 확인하지 못한 것 → 재현 방법.
- 결과에는 commit, image digest, 자원 제한, 입력, 동시 요청 수와 캐시 상태를 함께 적습니다. 평균 대신 trial별 p50/p95/p99를 구해 평균하고, 오류·중복 작업·복구 시간과 peak memory를 함께 봅니다.
- 유효성 기준은 측정 전에 정합니다. E1에서 start skew 108ms인 trial 하나는 사전 기준 100ms대로 제외하고 같은 조건의 보충 run을 실행했습니다. E3의 첫 AWS calibration은 CPU 포화, 두 번째는 공유 bucket의 이전 파생 이미지 때문에 무효 처리하고 기록만 남겼습니다.
- 분석기는 요청별 CSV, Prometheus counter, 자원 sample과 log를 먼저 교차 검증한 뒤 표와 SVG를 만듭니다. 보고서의 분석 산출물은 같은 image의 독립 재분석에서 SHA-256이 같은지 확인했습니다. 이 README의 그래프 세 장도 [원자료에서 다시 그리며](docs/figures/README.md) test가 최신 여부를 확인합니다.
...
서비스에는 Go 1.26.7과 libvips 8.16.1 개발 파일이 필요합니다. 버전을 고정한 Docker build가 재현 경로입니다.
